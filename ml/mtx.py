...
def get_frames(filename, n_frames=1):
    ...
    # cv2.CAP_PROP_FRAME_COUNT counts the tot. no. of frames in a video
    v_len = int(v_cap.get(cv2.CAP_PROP_FRAME_COUNT))
    # np.linspace returns evenly spaced numbers over a specified interval- numpy.linspace(start, stop, num=no. of nos. to generate, endpoint=True, retstep=False, dtype=None, axis=0)
    frame_list = np.linspace(0, v_len-1, n_frames+1, dtype=np.int16)
    # both start and stop included
    for fn in range(v_len):
        # Reading ( cam. read() ) from a VideoCapture returns a tuple (return value, image) . With the first item you check whether the reading was successful, and if it was then you proceed to use the returned image .
...
def prediction(path):
    ...
    train_ds = VideoDataset(ids=["public/frames"], transform=train_transformer)
